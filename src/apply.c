...
static bool RunUserHook(const char* wall_path, const char* relX_str, const char* relY_str) {
	...
	int n = snprintf(hook_path, sizeof(hook_path), "%s/.config/hypr-wallpicker/apply-wallpaper.sh", home);
...
static bool RunBuiltinFallback(const char* wall_path, const char* relX_str, const char* relY_str) {

	char script[] = "if command -v awww >/dev/null 2>&1; then "
	...
	execl("/bin/sh", "sh", "-c", script, "--", wall_path, relX_str, relY_str, NULL);
