...
char* BuildCacheImagePath(const char* cache_dir, const char* filename) {
	...
	size_t base_len = (dot != NULL && dot != filename) ? (size_t)(dot - filename) : strlen(filename);

	int needed = snprintf(NULL, 0, "%s/%.*s.png", cache_dir, (int)base_len, filename);
	if (needed < 0) {
	...
	snprintf(out, (size_t)needed + 1, "%s/%.*s.png", cache_dir, (int)base_len, filename);
	return out;
}
...
bool IsSupportedWallpaperFile(const char* filename) {
	return HasExtension(filename, ".png") || HasExtension(filename, ".jpg") || HasExtension(filename, ".jpeg");
}
